...
class StreetSignRecognizer(object):
    ...
    def __init__(self):
        ...
        self.hsv_lb = np.array([27, 154, 142]) # hsv lower bound
    ...
    def sign_bounding_box(self):
        ...
        """
        cv2.cvtColor(self.cv_image,cv2.COLOR_BGR2HSV,self.hsv_image)
        self.binary_image = cv2.inRange(self.hsv_image, self.hsv_lb, self.hsv_ub)
        # print np.shape(self.binary_image)
        top = 480
        left = 640
        bottom = 0
        right = 0
        for row in range(0,480,2):
            for col in range(0,640,2):
                if self.binary_image[row,col] != 0:
                    if row<top:
                        top = row
                    if row>bottom:
                        bottom = row
