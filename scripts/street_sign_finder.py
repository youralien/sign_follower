...
class StreetSignFinder(object):
    """ This robot should find street signs """


    def __init__(self):
        """ Initialize the street sign finder """
        rospy.init_node('street_sign_finder')
        self.cv_image = None                        # the latest image from the camera
        self.bridge = CvBridge()                    # used to convert ROS messages to OpenCV
        cv2.namedWindow('video_window')
        #self.slider_window = HSVSliderWindow()
        rospy.Subscriber("/camera/image_raw", Image, self.process_image)
        self.cropped_sign_publisher = rospy.Publisher("sign_follower/cropped_sign", Image, queue_size=10)
        ...
        while not rospy.is_shutdown():
            if not self.cv_image is None:
                if not self.cropped_sign is None:
                    self.cropped_sign_publisher.publish(
                        self.bridge.cv2_to_imgmsg(
                            self.cropped_sign, encoding="passthrough"
                            )
                        )
